...
class LCIMLocalStorage extends SQLiteOpenHelper {
     ...
     */
    private static final String DB_NAME_PREFIX = "LeanCloud.db";
    ...
    /**
     * 因为 onCreate 为初始化 db 的时候才调用的，
     * 所以多表的情况下需要主动调用此函数来创建表
     */
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (isIgnoreUpgrade(oldVersion, newVersion)) {
            db.execSQL(String.format(SQL_DROP_TABLE, tableName));
            onCreate(db);
        }
    }

    protected boolean isIgnoreUpgrade(int oldVersion, int newVersion) {
