...
public class LCIMProfileCache {
    ...
    public synchronized void initDB(Context context, String clientId) {
        profileDBHelper = new LCIMLocalStorage(context, clientId, "UserCache_4");
    }
...
//添加对话人的id
    ...
    private ContactUserDisplayBean getUserProfileFromJson(String str) {
        ...
        Date dateOfBirth = DateUtils.getDateConversion(jsonObject.getString(USER_DATA), null);
...
//            jsonObject.put(USER_DATA, String.valueOf(userProfile.getDateOfBirth().getTime()));//date的负值转换
//        } else {

        jsonObject.put(USER_DATA, DateUtils.getDateText(userProfile.getDateOfBirth()));//date的负值转换
