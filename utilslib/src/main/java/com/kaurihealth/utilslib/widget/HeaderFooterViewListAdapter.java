...
public class HeaderFooterViewListAdapter implements WrapperListAdapter, Filterable {
    ...
    private final ArrayList<GridViewFixedViewInfo> mHeaderItemInfo;
    private final ArrayList<GridViewFixedViewInfo> mFooterItemInfo;
    static final ArrayList<GridViewFixedViewInfo> EMPTY_INFO_LIST = new ArrayList<>();
    ...
    public HeaderFooterViewListAdapter(ArrayList<GridViewFixedViewInfo> headerItemInfo, ArrayList<GridViewFixedViewInfo> footerItemInfo,
                                       ListAdapter adapter, List<View> headerViews, List<View> footerViews, int numColumns) {
        mAdapter = adapter;
        if (headerViews == null) {
    ...
    private boolean areAllListInfoSelectable(ArrayList<GridViewFixedViewInfo> info) {
        if (mHeaderViews != null && mHeaderViews.size() > 0)
            return false;
        if (mFooterViews != null && mFooterViews.size() > 0)
            return false;
        if (info != null) {
            for (GridViewFixedViewInfo i : info) {
                if (!i.isSelectable) {
                    return false;
                }
            }
    ...
    public boolean removeHeaderItem(View v) {
        for (int i = 0; i < mHeaderItemInfo.size(); i++) {
            GridViewFixedViewInfo info = mHeaderItemInfo.get(i);
            if (info.view == v) {
    ...
    public boolean removeFooterItem(View v) {
        for (int i = 0; i < mFooterItemInfo.size(); i++) {
            GridViewFixedViewInfo info = mFooterItemInfo.get(i);
            if (info.view == v) {
