...
public class NineGridImageView<T> extends ViewGroup {
    ...
    private void layoutChildrenView() {
        if (mImgStringList == null) {
            return;
        }
        int childrenCount = mImgStringList.size();
        for (int i = 0; i < childrenCount; i++) {
            ImageView childrenView = (ImageView) getChildAt(i);
            if (mAdapter != null) {
                mAdapter.onDisplayImage(getContext(), childrenView, mImgStringList.get(i));
            }
    ...
    public void setImagesData(List<String> lists) {
        ...
        if (mImgStringList == null) {
            ...
            int oldViewCount = mImgStringList.size();
    ...
    protected static int[] calculateGridParam(int imagesSize) {
        ...
        if (imagesSize < 3) {//1、2张时
            gridParam[0] = 1;
            gridParam[1] = imagesSize;
        } else if (imagesSize <= 4) {//4张时
            gridParam[0] = 2;
            gridParam[1] = 2;
        } else {
     ...
     * @param adapter 适配器
     */
    public void setAdapter(NineGridImageViewAdapter<String> adapter) {
        mAdapter = adapter;
    }
