 ...
 * <p/>
 * Describe: 实验室记录通用view
 */

class LobRecordCommonView extends AbstractViewController<PatientRecordDisplayDto> implements CollapsibleGroupView.ChildGroupView {
    ...
    RecentlyRecordLayout lay_project;

    private LinearLayout mMeasurement;
    private String mTitle;

    LobRecordCommonView(Context context, String title) {
        super(context);
        this.mTitle = title;
    }
...
//        mLayHospitalNumber.contentText(CheckUtils.checkTextContent(bean.getHospitalNumber()));
        lay_project.contentText(CheckUtils.checkTextContent(bean.getLabTest().getLabTestType()));

        mLayDocuments.initDocumentPathAdapter(bean.getRecordDocuments());
    ...
    public int getChildResLayoutId() {
        return R.layout.include_collapsible_group_view_lob;
    }
    ...
    public void onCreateChildView(View view) {
        ...
        lay_project = (RecentlyRecordLayout) view.findViewById(R.id.lay_project);
    }
