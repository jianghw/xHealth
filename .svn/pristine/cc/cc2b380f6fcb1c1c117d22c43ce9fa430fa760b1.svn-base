...
public class TimeDialog extends Dialog {
    ...
    public TimeDialog(final Context context) {
        super(context, R.style.Dialog_Mrecord);
        View view = LayoutInflater.from(context).inflate(R.layout.dialog_m_record, null);
        setContentView(view);
        ...
        window.setWindowAnimations(R.style.Animation_Dialog);  //添加动画
...
//        numpick_left.setDisplayedValues(null);
    ...
    private static void setNumberPickerDividerColor(NumberPicker numberPicker, Context context) {
        ...
        for (Field pf : pickerFields) {
            if (pf.getName().equals("mSelectionDivider")) {
                ...
                try {
                    //设置分割线的颜色值
                    pf.set(picker, new ColorDrawable(context.getResources().getColor(R.color.color_gray)));
    ...
    public interface TimeSelect {
        void selectTime(String date, String time);
    }
}
