...
public class NetWorkMedicalConsultationOnlyReadActivity extends BaseActivity {
    ...
    private static  final int NETWORKMEDICALCONSULTATION = 5;
    ...
    public void  onMoreClick(){
        if (mTvMore.getText().equals("编辑")){
            EventBus.getDefault().postSticky(new OutpatientElectronicBeanEvent(mPatientRecordDisplayBean,relationshipBean));
            skipToForResult(NetworkMedicalConsultationActivity.class,null,NETWORKMEDICALCONSULTATION);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == NETWORKMEDICALCONSULTATION && resultCode ==RESULT_OK){
            finishCur();
        }
    }
