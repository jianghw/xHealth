...
public class GetMoneyOutFragmentPresenter<V> implements IGetMoneyOutFragmentPresenter<V> {

    private final IDataSource mRepository;
    private CompositeSubscription mSubscriptions;
    private IGetMoneyOutFragmentView mFragment;
    List<CreditTransactionDisplayBean> displayBeen = new ArrayList<>();

    @Inject
    GetMoneyOutFragmentPresenter(IDataSource repository) {
        mRepository = repository;
        mSubscriptions = new CompositeSubscription();
    }
    @Override
    public void setPresenter(V view) {
        mFragment = (IGetMoneyOutFragmentView) view;
    }
