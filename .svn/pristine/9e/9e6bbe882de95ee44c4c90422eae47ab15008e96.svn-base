 ...
 */
public class AddAndEditLobTestPresenter<V> implements IAddAndEditLobTestPresenter<V> {

    private final IDataSource mRepository;
    private CompositeSubscription mSubscriptions;
    private IAddAndEditLobTestView mActivity;

    @Inject
    public AddAndEditLobTestPresenter(IDataSource mRepository){
        this.mRepository = mRepository;
        mSubscriptions = new CompositeSubscription();
    }
    @Override
    public void setPresenter(V view) {
        mActivity = (IAddAndEditLobTestView) view;
    }
    ...
    private void updatePatientRecord(PatientRecordDisplayBean patientRecordDisplayBean) {
        Subscription subscription = mRepository.UpdateLabTest(patientRecordDisplayBean)
    ...
    private void addDataProcessing(List<DocumentDisplayBean> beanList, ArrayList<String> paths) {
        NewLabTestPatientRecordDisplayBean newLabTestPatientRecordDisplayBean = mActivity.getNewLabTestPatientRecordDisplayBean();
        LogUtils.jsonDate(newLabTestPatientRecordDisplayBean);
        List<NewLabTestPatientRecordDisplayBean.RecordDocumentsBean> list = new ArrayList<>();
        //添加本地图片
        if (beanList != null && beanList.size() > 0){
            for (DocumentDisplayBean documentDisplayBean : beanList) {
                NewLabTestPatientRecordDisplayBean.RecordDocumentsBean newDocumentDisplayBean =
                        new NewLabTestPatientRecordDisplayBean.RecordDocumentsBean();
            ...
            newLabTestPatientRecordDisplayBean.setRecordDocuments(list);
        }
        addNewRecord(newLabTestPatientRecordDisplayBean);
    }

    private void addNewRecord(NewLabTestPatientRecordDisplayBean newLabTestPatientRecordDisplayBean) {
        Subscription subscription = mRepository.InsertLabTest(newLabTestPatientRecordDisplayBean)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<PatientRecordDisplayBean>() {
                    @Override
